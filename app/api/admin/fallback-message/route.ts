...
export async function GET(req: NextRequest) {
  ...
  const { data: settings, error } = await supabase
    .from("fallback_message_settings")
    .select("*")
    .eq("id", true)
    .single();
  if (error || !settings) {
    return NextResponse.json(
      safeServerError("admin.fallback_message_load_failed", error ?? new Error("no settings row"), undefined, "Failed to load settings"),
      { status: 500 }
    );
  }
...
export async function PUT(req: NextRequest) {
  ...
  // Enabling requires an actual message -- otherwise every non-/start
  // message would silently get an empty reply, which looks broken rather
  // than off.
  if (enabled && !html.trim()) {
    return NextResponse.json({ error: "Message is required to enable the fallback message" }, { status: 400 });
  }
  ...
  if (error || !settings) {
    return NextResponse.json(
      safeServerError("admin.fallback_message_save_failed", error ?? new Error("no settings row"), undefined, "Failed to save settings"),
      { status: 500 }
    );
  }
