...
// Same editing pattern as WelcomeMessageEditor -- this is its own settings
// row (fallback_message_settings) since it fires on a different trigger:
// any private message that ISN'T a bare /start, rather than /start itself.
export function FallbackMessageEditor() {
  ...
  useEffect(() => {
    fetch("/api/admin/fallback-message")
      .then((res) => res.json())
      .then((data) => {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    if (enabled && !messageHtml.trim()) {
      setSaveError("Message is required to enable the fallback message");
      return;
    }
    if (overLimit) {
    ...
    setSaving(true);
    try {
      const res = await fetch("/api/admin/fallback-message", {
        method: "PUT",
  ...
  return (
    <form onSubmit={handleSave} className="space-y-4 rounded-lg border border-white/10 bg-white/5 p-4">
      <div className="flex items-center justify-between">
        <p className="text-sm font-medium text-white">Fallback Message</p>
      ...
      <p className="text-xs text-white/40">
        Sent automatically when a user sends any private message that isn&apos;t a plain{" "}
        <code className="text-white/60">/start</code> — the bot has no other commands, so without this, anyone who
        types something else gets silence.
      </p>
      ...
      <div>
        ...
        <textarea
          ...
          rows={5}
          placeholder={"👋 Not sure what you meant, but here's the app!\n\nTap below to open Raffly."}
